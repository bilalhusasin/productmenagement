...
class Subjects extends MX_Controller {
    ...
    // this function edit subject title and writer by class id
    public function classSubjectEdit()
    {
        $class_id = $this->input->get('c_id');
        $subject_id = $this->input->get('s_id');
        if($this->input->post('submit', TRUE)){
            $class_id=$this->input->post('class_id', TRUE);
            $subject_id=$this->input->post('subject_id', TRUE);
            $upload_data = array(
                'subject_title' => $this->db->escape_like_str($this->input->post('subject_title', TRUE)),
                'writer_name' => $this->db->escape_like_str($this->input->post('writer_name', TRUE)),
                // 'id' => $this->db->escape_like_str($this->input->post('op_sub', TRUE)),
                // 'class_id' => $this->db->escape_like_str($this->input->post('op_sub', TRUE)),
            ); 
           $this->db->where('id', $subject_id);
            if ($this->db->update('class_subject', $upload_data)) {

                $query = $this->db->query("SELECT * FROM class_subject WHERE id='$subject_id' AND class_id='$class_id'");
                $data['subInfo'] =($query->result_array());
                $data['message'] = '<div class="alert alert-success alert-dismissable">
                                <button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
                                <strong>'.lang('success').'</strong> '."Subject Successfully Edit in subject list.".'
                            </div>';
            ...
            $data['subInfo'] =($query->result_array());
            $this->load->view('temp/header');
            $this->load->view('classSubjectEdit', $data);
            $this->load->view('temp/footer');
        }
    }
    // this function delete subject title
    public function classSubjectDelete(){ 
        $class_id = $this->input->get('c_id');
        $subject_id = $this->input->get('s_id'); 
         
        if($this->db->delete('class_subject', array('id' => $subject_id))){
            $data['SubjectInfo'] = $this->common->getAllData('class');
            $data['message'] = '<div class="alert alert-success alert-dismissable">
                                <button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
                                <strong>Success ! </strong> Subject Info delete successfully. 
                            </div>';
            $this->load->view('temp/header');
            $this->load->view('subjectInformation', $data);
            $this->load->view('temp/footer'); 
