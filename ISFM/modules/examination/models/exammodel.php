...
//This function is returan Exam Title by ExamId.
public function studentname($stid) {
$data = array();
$query2 = $this->db->query("SELECT student_title FROM class_students WHERE student_id=$stid");
foreach ($query2->result_array() as $row) {
return $row['student_title'];
}
}
...
public function previewAttendance($a, $b, $c, $d) {
$data = array();
$query = $this->db->get_where('exam_attendanc', array('class_id' => $a, 'exam_title' => $b, 'exam_subject' => $c, 'section' => $d));
foreach ($query->result_array() as $row) {
$data[] = $row;
}return $data;
}
...
public function markshit($examId, $class_id, $section, $studentId) {
$data = array();
$query = $this->db->get_where('result_shit', array('exam_id' => $examId, 'class_id' => $class_id, 'section'=>$section, 'student_id' => $studentId));
foreach ($query->result_array() as $row) {
$data[] = $row;
}return $data;
}
//This function return markshit by examtitle,class title and student id.
public function fullResult($class_id, $section, $examId) {
$data = array();
$querystatus=$this->db->query("SELECT student_id from student_info WHERE class_id='$class_id' AND section='$section' AND status='Active'");
foreach ($querystatus->result_array() as $row) {
$student_id = $row['student_id'];
}
$query = $this->db->query("SELECT * FROM `result_shit` WHERE `class_id`= '$class_id' AND `exam_id` = '$examId'  AND  `section` = '$section' AND student_id= '$student_id' ORDER BY student_id");

/*$query = $this->db->get_where('result_shit', array('class_id' => $class_id,'section'=>$section, 'exam_id' => $examId));*/
...
//This function return markshit by examtitle,class title and student id.
public function overallmarkshit($class_id, $studentId) {
$data = array();
$query = $this->db->get_where('result_shit', array('class_id' => $class_id, 'student_id' => $studentId));
foreach ($query->result_array() as $row) {
$data[] = $row;
}return $data;
}
//This function return markshit by examtitle,class title and student id.
/*
public function overallmarkshit($examId,$class_id, $studentId) {
$data = array();
