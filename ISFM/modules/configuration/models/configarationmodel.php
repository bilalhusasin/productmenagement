...
class Configarationmodel extends CI_Model {
    ...
    public function employ() {
        $data = array();
        $query = $this->db->query("SELECT id,username FROM users WHERE user_status='Employee' AND salary=0 AND status='Active'"); //active=1
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
...
//this function will return all employe fromusers table
    ...
    public function allGroup() {
        $data = array();
        $query = $this->db->query("SELECT id,name FROM groups WHERE status='Active'");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    //This function will return fee structure
    public function edit_fee_stru($id) {
        $data = array();
        $query = $this->db->query("SELECT * FROM fee_structure WHERE id=$id");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }return $data;
    }
