...
//                echo '</pre>';
    ...
    public function checkroom(){
        $value = $this->input->get('val');
        $query = $this->db->query("SELECT rome_number FROM exam_routine WHERE rome_number='$value'");
        foreach($query->result_array() as $row){
            $room = $row['rome_number'];
        }
        if(!empty($room)){
            echo '<div class="row"><div class="alert alert-danger">
                    <strong>Notic: </strong> This Room <b>'.$value.'</b> have in our database from past. Add Other Room Number.
                </div></div>';
        }
    }
