...
class HomeModel extends CI_Model {
    ...
    public function atten_chart_students() {
        $data = array();
        $query = $this->db->query("SELECT count(class_title) as id, class_title as count_year from student_info where class_title IS NOT NULL group by class_title");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    public function atten_chart_students1() {
        $data = array();
        $query = $this->db->query("SELECT count(reg_date) as id, reg_date as count_year from registration group by reg_date");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    public function atten_chart_students_reg() {
        $data = array();
        $query = $this->db->query("SELECT class.class_title as title,  SUM(registration_fee) as sumz from registration INNER JOIN class ON registration.class_id = class.id group by class.class_title ORDER BY class.id");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    public function atten_chart_students2() {
        $data = array();
        $query = $this->db->query("SELECT count(month) as id, month as count_year from slip group by month");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    public function atten_chart_students_admision() {
        ...
        $data = array();
        // previous query
        // $query = $this->db->query("SELECT SUM(dis_total) as ss, month from slip where payment_status='Paid' group by month ORDER BY FIELD(MONTH, 'January', 'February', 'March', 'Ápril', 'May', 'June', 'July', 'August','September','Óctober', 'November','December')");

        // this one is new query for graph for admission fee
        // , SUM((register_pass.admission_fee + register_pass.annual_found) - register_pass.admission_disc) as ss,
        $query = $this->db->query("SELECT SUM(register_pass.total) as ss, MONTH(register_pass.reg_date) as month FROM student_info INNER JOIN register_pass ON student_info.registration_number = register_pass.reg_number WHERE student_info.year = $year AND register_pass.year = $year group by MONTH(register_pass.reg_date)");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    /*public function atten_chart_students3() {
        $data = array();
        $query = $this->db->query("SELECT class.class_title, count(*) as qty FROM slip  INNER JOIN class ON slip.class_id = class.id where slip.discount_persentage !=0 GROUP BY     class_title HAVING count(*)> 1");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
...
////// leftover students /////
    ...
    public function atten_chart_student_feeStructure() {
        $data = array();
        $query = $this->db->query("SELECT sum(slip.dis_total) as sumz , slip.class_id,class.class_title from slip INNER JOIN class ON slip.class_id= class.id where slip.payment_status='Paid' group by slip.class_id ");
