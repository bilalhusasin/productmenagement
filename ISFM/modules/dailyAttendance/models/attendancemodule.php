...
class AttendanceModule extends CI_Model {
    ...
    public function teacherList() {
        $data = array();
        $query = $this->db->query("SELECT user_id,full_name FROM userinfo WHERE status ='Active'");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    public function teach($u_id){
        $data = array();
        $query = $this->db->query("SELECT full_name FROM userinfo WHERE user_id ='$u_id' AND status = 'Active'")->row();
