...
//    private $input;
        ...
        if ($this->input->post('submit', TRUE)) {
            ...
            if ($this->input->post('section', TRUE)) {
                ...
                if ($Section == 'all') {
                    ...
                    $query = $this->db->query("SELECT * From class_students WHERE `class_title`='$classTitle' AND (`status`='Active' OR `status`='Defaulter')");
                    ...
                    if (!empty($data['students'])) {
                        ...
                        echo $classTitle . ' and all section are no any student record.';
                    }
                } elseif ($Section != 'all') {
                    //if want All section's students attendence sheet,then work this erea.
                    $queryData = array();
                    //$query = $this->db->get_where('class_students', array('class_title' => $classTitle, 'section' => $Section, 'status'=>'Active' ));
                    $query = $this->db->query("SELECT * From class_students WHERE `class_title`='$classTitle' AND `section`='$Section' AND (`status`='Active' OR `status`='Defaulter')"); 
                    foreach ($query->result_array() as $row) {
                        $queryData[] = $row;
                    }
                    $data['students'] = $queryData;
                    ...
                    if (!empty($data['students'])) {
                        ...
                        echo 'Class ' . $classTitle . ' and ' . $Section . ' section are no any student record.';
                    }
                }
            } else {
               ...
               // $query = $this->db->get_where('class_students', array('class_title' => $classTitle));
                ...
                if (!empty($data['students'])) {
                    ...
                    echo $classTitle . ' are no any student record.';
                }
            }
        }
    }
        ...
        if ($this->input->post('section', TRUE)) {
            ...
            if ($Section == 'all') {
                ...
                if (!empty($data['students'])) {
                    ...
                    echo $classTitle . ' and ' . $Section . ' section are no any student record.';
                }
            }
        } else {
            //whene want any class students attendence sheet only,then work this erea.
    ...
    public function teacherAttendance() {
        if ($this->input->post('submit', TRUE)) {
           ...
           // $data['teacher'] = $this->attendancemodule->teacherList();
            ...
            $data['group_title']=$query->result_array();
    ...
    //This function send group user using group id to view by ajax. 
    public function selectUsergroup() {
        $groupId = $this->input->get('q', TRUE);
        // echo $groupId;
        $query = $this->db->query("SELECT * FROM userinfo WHERE group_id='$groupId' AND status ='Active'");
        // $query = $this->common->getWhere('class', 'class_title', $classTitle);
       ...
       // echo '<input type="hidden" name="class" value="' . $classTitle . '">';
        ...
        echo '<div class="form-body">
            ...
            $section = 'This group has no Record.';
            echo'<div class="form-group">
                    <label class="col-md-3 control-label"></label>
                    <div class="col-md-6">
                        <div class="alert alert-warning">
